...
from pipeline_custom.pipeline_noise_collage_controlnet import NoiseCollageControlnetStableDiffusionPipeline
from pipeline_custom.pipeline_noise_collage_controlnet import make_pipe, make_pipe_from_file, format_condition_mscoco
...
class ExpConfigs:
    ...
    model_controlnet:str = "sketch"  # pose, sketch ... 
    ...
    ## MS-COCO settings
    ms_coco_path:str = "./sample_inputs/controlnet-sketch"
    prompt_type:str = 'self_caption' # <-- 'self_caption' or 'blip_caption'

    ## Generation Settings
    collage_mode:str = "fixed"  # <-- ablation study (default: "fixed")
    batch_size:int = 4
    resolution:int = 512
...
def main():
    ...
    if '_safetensors' in model_name:
        pipe:NoiseCollageControlnetStableDiffusionPipeline = make_pipe_from_file(args, cls=NoiseCollageControlnetStableDiffusionPipeline)
    else:
        pipe:NoiseCollageControlnetStableDiffusionPipeline = make_pipe(args, cls=NoiseCollageControlnetStableDiffusionPipeline)
    ...
    setattr(args, 'latents', torch.randn((args.batch_size,4,64,64), device=f'cuda:{args.GPU_IDX}')) # fix latents if you need.
